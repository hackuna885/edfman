...
<div class="marcoP">
        <table style="width: 2426px;">
            <thead>
                <tr>
                    <th colspan="35" rowspan="0" style="background-color: transparent; color: #1F419A;"><h2 style="text-align: center; padding: 5px; margin: 10px;">Matriz Individual GII</h2></th>
                </tr>
            </thead>
            <thead>
                <tr>
		...
		<button onclick="exportTableToExcel()">Descargar tabla como Excel</button>

    </div>
	<script>
		function exportTableToExcel(filename = 'tabla-exportada.xls') {
			// Obtener la tabla HTML
			let tablaHTML = document.querySelector('table').outerHTML;
			
			// Crear un archivo Blob con el contenido de la tabla
			let blob = new Blob(['<html><head><meta charset="UTF-8"></head><body>' + tablaHTML + '</body></html>'], {
